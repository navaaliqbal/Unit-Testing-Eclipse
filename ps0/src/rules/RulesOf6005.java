...
public class RulesOf6005 {
	...
	public static boolean mayUseCodeInAssignment(boolean writtenByYourself,
	    ...
	    if (availableToOthers) {
	        // If the code is available to others, the source must be cited 
	        if (citingYourSource) {
	            return true; // Allowed if properly cited
	        }
	        return false; // Not allowed if not cited
	    }
	    
	    // If it was written as coursework and implementation is required, it can't be used
	    if (writtenAsCourseWork && implementationRequired) {
	        return false; // Not allowed since it requires original work
	    }
